...
*Setup instructions for a gitian build of CipherCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CipherCoin
...
- [Building CipherCoin](#building-ciphercoin)
...
Clone the git repositories for ciphercoin and gitian and then checkout the ciphercoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ciphercoin-crypto/ciphercoin.git
cd ciphercoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ciphercoin repository
...
To build CipherCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ciphercoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ciphercoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ciphercoin-crypto/ciphercoin
...
For example:
```bash
URL=https://github.com/crowning-/ciphercoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ciphercoin=${COMMIT} --url ciphercoin=${URL} ../ciphercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ciphercoin=${COMMIT} --url ciphercoin=${URL} ../ciphercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ciphercoin=${COMMIT} --url ciphercoin=${URL} ../ciphercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ciphercoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ciphercoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ciphercoin-build.assert
```
...
[ciphercoin/gitian.sigs](https://github.com/ciphercoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
