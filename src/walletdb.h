...
public:
    ...
    bool WriteZCIPHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCIPHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCIPHSeed_deprecated(uint256& seed);
    bool EraseZCIPHSeed();
    bool EraseZCIPHSeed_deprecated();

    bool WriteZCIPHCount(const uint32_t& nCount);
    bool ReadZCIPHCount(uint32_t& nCount);
