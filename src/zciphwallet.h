...
#ifndef CipherCoin_ZCIPHWALLET_H
#define CipherCoin_ZCIPHWALLET_H
...
public:
    CzCIPHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCIPH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCIPH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CipherCoin_ZCIPHWALLET_H
