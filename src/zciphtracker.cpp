...
CzCIPHTracker::CzCIPHTracker(std::string strWalletFile)
{
...
CzCIPHTracker::~CzCIPHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCIPHTracker::Init()
{
...
bool CzCIPHTracker::Archive(CMintMeta& meta)
{
...
bool CzCIPHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCIPHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCIPHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCIPHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCIPHTracker::GetSerialHashes()
{
...
CAmount CzCIPHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCIPHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCIPHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCIPHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCIPHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCIPHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCIPHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCIPHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCIPHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCIPHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCIPHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCIPHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCIPHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCIPHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCIPHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCIPHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCIPHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzciph from DB\n", __func__, listDeterministicDB.size());
    }
