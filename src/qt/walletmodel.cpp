...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ciphercoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CIPH.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ciphercoin:URI (ciphercoin:XyZ...?message=example)
            {
...
static void NotifyzCIPHReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCIPHReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCIPHReset.connect(boost::bind(NotifyzCIPHReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCIPHReset.disconnect(boost::bind(NotifyzCIPHReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zciphTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
