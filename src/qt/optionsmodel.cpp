...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CIPH);
    ...
    if (!settings.contains("nAnonymizeCipherCoinAmount"))
        settings.setValue("nAnonymizeCipherCoinAmount", 1000);

    nAnonymizeCipherCoinAmount = settings.value("nAnonymizeCipherCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCipherCoinAmount"))
        SoftSetArg("-anonymizeciphercoinamount", settings.value("nAnonymizeCipherCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ciphercoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCipherCoinAmount:
            return QVariant(nAnonymizeCipherCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCipherCoinAmount:
            nAnonymizeCipherCoinAmount = value.toInt();
            settings.setValue("nAnonymizeCipherCoinAmount", nAnonymizeCipherCoinAmount);
            emit anonymizeCipherCoinAmountChanged(nAnonymizeCipherCoinAmount);
            break;
        case CoinControlFeatures:
