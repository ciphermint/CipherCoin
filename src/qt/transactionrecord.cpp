...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zciphTracker->HasMintTx(hash))) {
            //zCIPH stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZCIPH;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by CipherCoin Address
...
bool IsZCIPHType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZCIPH:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZCIPH || type == TransactionRecord::MNReward) {
