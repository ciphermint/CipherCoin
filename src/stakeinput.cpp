...
// The zCIPH block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCIPH is a hash of the serial
...
{
    //Create an output returning the zCIPH that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCIPHOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCIPH output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCIPH", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCIPHOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCIPH output", __func__);
...
{
    CzCIPHTracker* zciphTracker = pwallet->zciphTracker.get();
    CMintMeta meta;
    if (!zciphTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zciphTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CIPH stake is the outpoint
