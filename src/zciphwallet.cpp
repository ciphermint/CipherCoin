...
CzCIPHWallet::CzCIPHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zciph seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCIPHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCIPHSeed_deprecated()) {
                    LogPrintf("%s: Updated zCIPH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zciph seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zciph wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCIPHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCIPHCount(nCountLastUsed);
    else if (!walletdb.ReadZCIPHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCIPHWallet::Lock()
{
    seedMaster = 0;
}

void CzCIPHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCIPHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCIPH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCIPHWallet::LoadMintPoolFromDB()
{
...
void CzCIPHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCIPHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCIPHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zciphTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCIPHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCIPH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zciphTracker which also adds to database
    pwalletMain->zciphTracker->Add(dMint, true);
...
void CzCIPHWallet::SeedToZCIPH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCIPHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCIPHWallet::GenerateDeterministicZCIPH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCIPHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCIPHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
